...
if os.path.exists(os.path.join(model_save_path, 'faiss.index')):
    ...
    for x in trange(9, desc='Encoding corpus'):
        corpus_embeddings = model.encode(corpus[x * 1000000:(x + 1) * 1000000], convert_to_tensor=True,
    ...
    for i in trange(1, 10, desc='Creating index'):
        all_corpus = torch.load(os.path.join(model_save_path, f'corpus_tensor_{i}.pt'),
                                map_location=torch.device(device)).detach().cpu().numpy()
